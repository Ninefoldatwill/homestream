...
from modes import (
    DeployMode, FeatureFlag, ModeConfig, ModeValidator,
    get_mode_config, switch_mode, get_mode_info,
)
...
class TestGetModeConfig(unittest.TestCase):
    ...
    def test_get_solo_mode(self, mock_getenv):
        """测试获取Solo模式配置"""
        mock_getenv.side_effect = lambda key, default: "solo" if key == "OPENBRIDGE_MODE" else default
        # 需要清除缓存
    ...
    def test_get_team_mode(self, mock_getenv):
        """测试获取Team模式配置"""
        mock_getenv.side_effect = lambda key, default: "team" if key == "OPENBRIDGE_MODE" else default
        from modes import get_mode_config
        get_mode_config.cache_clear()
        config = get_mode_config()
        self.assertEqual(config.mode, DeployMode.TEAM)

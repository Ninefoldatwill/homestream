...
from event_stream import (
    Action, EventType, EventSource, Event,
    _gen_event_id, create_action, create_done_action,
    create_warn_action, create_ask_action, create_task_action,
    parse_handoff_text,
)
...
class QueryKnowledgeAction(Action):
    ...
    sources: List[str] = Field(
        default_factory=lambda: ["shuge", "knowledge_graph", "vector"],
        description="查询源"
    )
...
class UpdateLearningAction(Action):
    ...
    4. 晋升固化 → 封装为Skill
    """
    learning_type: str = Field(..., description="类型: error/correction/best_practice/feature_request")
    content: str = Field(..., description="学习内容")
    file_path: Optional[str] = Field(None, description="写入哪个文件")
...
def create_assign_task(orchestrator: str, builder: str,
                       task_id: str, title: str, description: str = "",
                       deadline: Optional[str] = None, 
                       priority: str = "medium",
                       confidence: Optional[float] = None) -> AssignTaskAction:
...
def create_handoff(task_id: str, build_agent: str, review_agent: str,
                   what_done: str, where_artifacts: List[str],
                   how_verify: str, known_issues: List[str],
                   what_next: str, confidence: Optional[float] = None) -> HandoffTaskAction:
...
def create_review(task_id: str, reviewer: str, orchestrator: str,
                  approved: bool, comments: Optional[str] = None,
                  issues_found: Optional[List[str]] = None,
                  score: Optional[int] = None) -> ReviewTaskAction:
...
def create_query_knowledge(sender: str, query: str,
                           sources: Optional[List[str]] = None,
                           top_k: int = 5) -> QueryKnowledgeAction:
...
def create_update_learning(sender: str, learning_type: str, content: str,
                           file_path: Optional[str] = None) -> UpdateLearningAction:

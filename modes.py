...
class ModeValidator:
    ...
    def validate(self) -> Dict[str, Any]:
        ...
        # 2. 检查必需功能是否启用
        required = MODE_FEATURE_MAP[DeployMode(self.config.mode) if isinstance(self.config.mode, str) else self.config.mode]
        ...
        if str(self.config.mode) == "ecosystem" and settings.port == 3458:
            warnings.append(
                f"Ecosystem模式推荐使用端口 8080，当前为 {settings.port}"
            )

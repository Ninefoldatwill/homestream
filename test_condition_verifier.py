...
def _mk_stream(session_id: str | None = None) -> EventStream:
...
class TestKappaCondition(unittest.TestCase):
    ...
    def test_no_convergence_when_outputs_vary(self):
        ...
        self.assertFalse(
            result.should_stop or result.condition == StopCondition.KAPPA,
    ...
    def test_kappa_triggers_on_convergent_outputs(self):
        ...
        cfg2 = VerifierConfig(
            kappa_window=3, kappa_threshold=0.95, max_iterations=999, empty_timeout=0
        )
...
class TestPhiCondition(unittest.TestCase):
    ...
    def test_phi_triggers_when_done_no_pending(self):
        ...
        cfg = VerifierConfig(max_iterations=999, empty_timeout=0, enable_deep_check=True)
        v = ConditionVerifier(cfg)
        result = v.check(stream)
...
class TestEmptyCondition(unittest.TestCase):
    ...
    def test_empty_does_not_trigger_immediately(self):
        ...
        stream = _mk_stream()
        cfg = VerifierConfig(
            empty_timeout=30.0,  # 30 秒
            max_iterations=999,
            enable_deep_check=False,
        )
        v = ConditionVerifier(cfg)
        result = v.check(stream)
        self.assertFalse(result.condition == StopCondition.EMPTY, "不应立即触发 EMPTY")

    def test_empty_disabled_when_zero(self):
        """empty_timeout=0 → 不启用 EMPTY 检测"""
        stream = _mk_stream()
        cfg = VerifierConfig(empty_timeout=0, max_iterations=999, enable_deep_check=False)
        v = ConditionVerifier(cfg)
        time.sleep(0.05)
        result = v.check(stream)
...
class TestErrorCondition(unittest.TestCase):
    ...
    def test_error_triggers_after_consecutive_errors(self):
        ...
        stream = _mk_stream()
        cfg = VerifierConfig(
            max_consecutive_errors=3, max_iterations=999, empty_timeout=0, enable_deep_check=False
        )
        v = ConditionVerifier(cfg)
        for _ in range(3):
    ...
    def test_error_resets_on_success(self):
        ...
        result = v.check(stream)
        self.assertFalse(result.condition == StopCondition.ERROR, "清零后不应触发 ERROR")
...
class TestMaxIterCondition(unittest.TestCase):
    ...
    def test_max_iter_triggers_exactly_at_limit(self):
        ...
        stream = _mk_stream()
        cfg = VerifierConfig(max_iterations=5, empty_timeout=0, enable_deep_check=False)
        v = ConditionVerifier(cfg)
        for i in range(5):
            result = v.check(stream)
        self.assertTrue(result.should_stop)
    ...
    def test_max_iter_disabled_when_zero(self):
        ...
        stream = _mk_stream()
        cfg = VerifierConfig(max_iterations=0, empty_timeout=0, enable_deep_check=False)
        v = ConditionVerifier(cfg)
        for _ in range(100):
            result = v.check(stream)
...
class TestContinueOnFail(unittest.TestCase):
    ...
    def test_no_stop_condition_returns_continue(self):
        """空流 + 未超时 + 无错误 → CONTINUE"""
        stream = _mk_stream()
        cfg = VerifierConfig(max_iterations=999, empty_timeout=999, enable_deep_check=True)
        v = ConditionVerifier(cfg)
        result = v.check(stream)
    ...
    def test_custom_checker_none_falls_through(self):
        ...
        stream = _mk_stream()
        cfg = VerifierConfig(max_iterations=999, empty_timeout=999, enable_deep_check=False)
        v = ConditionVerifier(cfg)
...
class TestVerifierSubscriber(unittest.TestCase):
    ...
    def _setup(self, cfg: VerifierConfig | None = None):
        stream = _mk_stream()
    ...
    def test_done_with_no_pending_triggers_stopped(self):
        ...
        stream, sub = self._setup(VerifierConfig(max_iterations=999, empty_timeout=0))
        ...
        stopped = [
            e
            for e in received_obs
            if e.sender == VerifierSubscriber.VERIFIER_AGENT_NAME and "[STOPPED]" in e.content
        ]
        self.assertTrue(
            len(stopped) > 0,
...
class TestWorktreeConfig(unittest.TestCase):
    ...
    def test_lenient_verifier_tolerates_more_errors(self):
        ...
        result = v.check(stream)
        self.assertFalse(
            result.condition == StopCondition.ERROR and result.should_stop,
            "宽松模式不应在第 5 次错误时停止",
        )
...
class TestFullLoopIntegration(unittest.TestCase):
    ...
    def test_agent_loop_continues_before_done(self):
        ...
        stream.publish(task)
        result = v.check(stream)
        self.assertFalse(result.should_stop, f"未发 DONE 不应停止，得到: {result}")

...
from event_stream import (
    ...
    create_action, create_done_action,
)
...
class ReviewerSubscriber:
    ...
    def __init__(self, stream: EventStream, manager: WorktreeManager):
        ...
        self.stream.subscribe_by_type(EventType.WARN, self._on_info_for_review)  # REVIEW:FAIL通过WARN事件传递
    ...
    def _on_done(self, event: Event):
        ...
        # 查找发送者的活跃Worktree
        sender_worktrees = [
            wt for wt in self.manager.list_worktrees()
                ...
                if conn:
                    try:
                        conn.execute(
                            ...
                            (event.event_id, "WARN", event.sender, event.recipient,
                             event.content, event.cause, datetime.now().isoformat())
                        )
                        conn.commit()
                    except Exception:
                        pass

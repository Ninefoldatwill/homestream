...
class WorktreeConfig:
    ...
    worktreeinclude_patterns: List[str] = field(default_factory=lambda: [
...
class PortManager:
    ...
    def __init__(self):
        ...
        self._canonical_owner: Optional[str] = None  # 当前持有规范端口的worktree
...
class SQLiteManager:
    ...
    def get_connection(self, worktree_name: str) -> Optional[sqlite3.Connection]:
        """获取Worktree的数据库连接"""
...
class WorktreeManager:
        ...
        with self._lock:
            ...
            try:
                ...
                if result.stdout.strip():
                    ...
                    # 创建新分支
                    cmd = ["git", "worktree", "add", "-b", branch_name, worktree_path, config.base_branch]
    ...
    def get_worktree(self, name: str) -> Optional[WorktreeConfig]:
        """获取指定Worktree配置"""
    ...
    def remove_worktree(self, name: str, force: bool = False) -> bool:
        ...
        with self._lock:
            ...
            if config.status == WorktreeStatus.LOCKED and not force:
                raise ValueError(
                    f"Worktree '{name}' 已锁定，不能删除。使用 force=True 强制删除。"
                )
            ...
            try:
                ...
                if force:
                    cmd.append("--force")
                subprocess.run(
                    cmd, capture_output=True, text=True, cwd=self.repo_path, timeout=15
                )
            except (subprocess.TimeoutExpired, FileNotFoundError):
                # git不可用，手动删除目录
    ...
    def lock_worktree(self, name: str, reason: str = "") -> bool:
        ...
        with self._lock:
            ...
            try:
                subprocess.run(
                    ["git", "worktree", "lock", config.worktree_path, "--reason", reason or "locked by WorktreeManager"],
                    capture_output=True, text=True, cwd=self.repo_path, timeout=10
                )
            except (subprocess.TimeoutExpired, FileNotFoundError):
                pass
    ...
    def _copy_config_files(self, config: WorktreeConfig):
        ...
        for pattern in config.worktreeinclude_patterns:
            ...
            for root, dirs, files in os.walk(self.repo_path):
                # 跳过.git和worktree目录
                dirs[:] = [d for d in dirs if d not in {'.git', 'worktrees', '__pycache__', '.worktree'}]
    ...
    def _save_worktree_meta(self, config: WorktreeConfig):
        ...
        meta = {
            ...
            "status": config.status.value if isinstance(config.status, WorktreeStatus) else config.status,
    ...
    def _load_worktree_meta(self, worktree_path: str) -> Optional[Dict]:
        """加载Worktree元数据"""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "reviewing": sum(1 for w in self._worktrees.values() if w.status == WorktreeStatus.REVIEWING),
            "locked": sum(1 for w in self._worktrees.values() if w.status == WorktreeStatus.LOCKED),
            "completed": sum(1 for w in self._worktrees.values() if w.status == WorktreeStatus.COMPLETED),
            "experimenting": sum(1 for w in self._worktrees.values() if w.status == WorktreeStatus.EXPERIMENTING),
            "ratchet_locked": sum(1 for w in self._worktrees.values() if w.status == WorktreeStatus.RATCHET_LOCKED),
            "rolled_back": sum(1 for w in self._worktrees.values() if w.status == WorktreeStatus.ROLLED_BACK),

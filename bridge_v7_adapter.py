...
from skillopt.evaluation.gate import evaluate_gate, GateAction, GateResult
...
from skill_router import SkillRouter, SkillEntry
...
class BridgeV7Adapter(EnvAdapter):
    ...
    def __init__(self, bridge_cfg: Optional[BridgeV7Config] = None):
        self.bridge_cfg = bridge_cfg or BridgeV7Config()
        self._cfg: dict = {}
        
        # 桥 v7 核心组件（延迟初始化）
        self._event_stream: Optional[EventStream] = None
        self._condition_verifier: Optional[ConditionVerifier] = None
        ...
        **kwargs,
        ...
        if failures:
            failure_patch = self._analyze_failures(
                failures, skill_content, out_dir,
            )
        ...
        if successes and self.bridge_cfg.enable_reviewer:
            success_patch = self._analyze_successes(
                successes, skill_content, out_dir,
            )
...
"""
    ...
    def _rule_based_failure_analysis(
        ...
        if "timeout" in fail_reasons or any("timeout" in r.get("fail_reason", "") for r in failures):
            edits.append(Edit(
                op="append",
                content="""## 超时处理
    ...
    def _apply_edits(self, skill_content: str, edits: list[Edit]) -> str:
        ...
        for edit in edits:
            if edit.op == "append":
                ...
                # 简化：替换包含 target 的整行
                new_lines = [
                    edit.content if edit.target in line else line
...
def create_bridge_v7_adapter(
    skillopt_cfg: Optional[dict] = None,
    bridge_cfg: Optional[BridgeV7Config] = None,
) -> BridgeV7Adapter:

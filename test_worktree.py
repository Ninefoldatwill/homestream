...
class TestSQLiteManager(unittest.TestCase):
    ...
    def test_02_isolation(self):
        ...
        conn_a.execute(
            "INSERT INTO worktree_state (key, value) VALUES (?, ?)", ("test_key", "value_a")
        )
        conn_a.commit()
        ...
        conn_b.execute(
            "INSERT INTO worktree_state (key, value) VALUES (?, ?)", ("test_key", "value_b")
        )
        conn_b.commit()

        # 验证隔离
        result_a = conn_a.execute(
            "SELECT value FROM worktree_state WHERE key=?", ("test_key",)
        ).fetchone()
        result_b = conn_b.execute(
            "SELECT value FROM worktree_state WHERE key=?", ("test_key",)
        ).fetchone()

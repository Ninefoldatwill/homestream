...
def test_event_stream_core(r: TestResult):
    ...
    r.ok(
        "Agent级订阅",
        f"收到{len(received)}条" if len(received) == 1 else f"期望1条，实际{len(received)}条",
    )
    ...
    stream.publish(evt3)
    r.ok(
        "类型级订阅", f"TASK事件{len(task_events)}条" if len(task_events) >= 1 else "未收到TASK事件"
    )
    ...
    r.ok(
        "完整因果链",
        f"链长{len(full_chain)}" if len(full_chain) >= 2 else f"链长{len(full_chain)}不正确",
    )
    ...
    r.ok(
        "ICP文本解析",
        f"type={parsed['event_type'].value}"
    ...
    r.ok(
        "ICP纯文本",
        f"type={parsed2['event_type'].value}"
        if parsed2["event_type"] == EventType.INFO
        else "默认类型错误",
    )
...
[What Next] 提交验收"""
    ...
    r.ok("Handoff解析", "5要素" if handoff and "what_done" in handoff else "解析失败")

    # 10. 统计信息
    stats = stream.get_statistics()
    r.ok(
        "统计信息", f"events={stats['total_events']}" if stats["total_events"] >= 3 else "统计错误"
    )

    # 11. ASK v1.1扩展
    ask = create_ask_action(
        "澜舟", "灵犀", "技术调研进度？", ask_id="ASK-001", context="v7集成", deadline="今日22:00"
    )
    ...
    r.ok(
        "WARN触发ERROR学习",
        f"learning_type={warn.learning_type}"
        if warn.learning_type == "error"
        else "WARN学习类型错误",
    )

    # 14. ICP v1.1输出格式
    icp_text = stream.to_icp_v1_format(done)
    r.ok("ICP v1.1格式输出", "含置信度" if "置信度" in icp_text else "缺少置信度标注")
...
def test_action_observation_types(r: TestResult):
    ...
    action_tests = [
        (
            "SendMessageAction",
            SendMessageAction(
                event_id="t1", event_type=EventType.INFO, sender="A", recipient="B", content="msg"
            ),
        ),
        (
            "AssignTaskAction",
        ...
        (
            ...
            UpdateLearningAction(
                ...
                content="learn",
                learning_type="error",
                learning_content="bug",
            ),
        ),
        (
            "PingAction",
            PingAction(
                event_id="t7", event_type=EventType.PING, sender="A", recipient="B", content="ping"
            ),
        ),
        (
            "LogAction",
    ...
    try:
        hnd = create_handoff(
            "T-101", "澜舟", "千寻", "完成开发", ["code.py"], "运行测试", [], "合并"
        )
...
def test_worktree_lifecycle(r: TestResult):
    ...
    r.ok(
        "锁定Worktree",
        f"status={wt_after_lock.status.value}"
        if wt_after_lock.status == WorktreeStatus.LOCKED
        else f"status={wt_after_lock.status.value}",
    )
    ...
    r.ok(
        "解锁Worktree",
        f"status={wt_after_unlock.status.value}"
        if wt_after_unlock.status == WorktreeStatus.ACTIVE
        else f"status={wt_after_unlock.status.value}",
    )
    ...
    r.ok(
        "统计信息",
        f"total={stats['total_worktrees']}" if stats["total_worktrees"] >= 1 else "统计错误",
    )
    ...
    r.ok("确认删除", "已删除" if wt_gone is None else "仍存在")
    ...
    r.ok(
        "PortManager双端口",
        f"wt-a bridge={ports1.get('bridge_v7')}, wt-b bridge={ports2.get('bridge_v7')}",
    )
...
def test_review_loop(r: TestResult):
    ...
    r.ok(
        "Worktree被锁定",
        f"status={wt.status.value}"
        if wt and wt.status == WorktreeStatus.LOCKED
        else f"status={wt.status.value if wt else 'None'}",
    )
    ...
    r.ok(
        "审查任务派发", f"count={len(review_tasks)}" if len(review_tasks) >= 1 else "未收到审查任务"
    )
    ...
    r.ok(
        "审查通过解锁",
        f"status={wt_after.status.value}"
        if wt_after and wt_after.status == WorktreeStatus.ACTIVE
        else f"status={wt_after.status.value if wt_after else 'None'}",
    )
    ...
    done2 = create_done_action(
        sender="澜舟",
        recipient="千寻",
        task_id="T-REVIEW-002",
        what_done="第二版开发",
        where_artifacts=["code2.py"],
        how_verify="运行测试",
        known_issues=["边界情况"],
        what_next="请审查",
        confidence=0.7,
    ...
    r.ok(
        "审查不通过解锁",
        f"status={wt2_after.status.value}"
        if wt2_after and wt2_after.status == WorktreeStatus.ACTIVE
        else f"status={wt2_after.status.value if wt2_after else 'None'}",
    )
...
def test_full_ecosystem_flow(r: TestResult):
    ...
    step1 = create_task_action(
        "九重", "澜澜", "请协调桥v7集成测试", "ECO-001", deadline="今日22:00"
    )
    ...
    step4 = create_done_action(
        sender="灵犀",
        recipient="澜澜",
        task_id="ECO-001-Sub1",
        what_done="集成测试调研完成",
        where_artifacts=["research.md"],
        how_verify="打开报告确认完整",
        known_issues=["需要适配v7"],
        what_next="建议使用pytest+FastAPI TestClient",
        confidence=0.85,
    )
    ...
    step6 = create_done_action(
        sender="澜舟",
        recipient="千寻",
        task_id="ECO-001-Sub2",
        what_done="桥v7集成测试开发完成",
        where_artifacts=["test_day3_integration.py"],
        how_verify="运行测试全部通过",
        known_issues=["性能待优化"],
        what_next="请审查后合并",
        confidence=0.9,
...
def test_v6_compatibility(r: TestResult):
    ...
    r.ok(
        "v6→v7→ICP往返",
        "包含TASK和九重→澜澜"
        if "[TASK]" in roundtrip and "九重→澜澜" in roundtrip
        else f"往返失真: {roundtrip}",
    )
    ...
    r.ok(
        "置信度标注",
        "含置信度" if "80%" in icp_conf or "置信度" in icp_conf else f"缺少: {icp_conf}",
    )
    ...
    r.ok(
        "ASK v1.1标注",
        "含id" if "ASK-V6-001" in icp_ask or "id:" in icp_ask else f"缺少id: {icp_ask}",
    )
...
def test_performance_benchmark(r: TestResult):
    ...
    r.ok(
        "单事件发布延迟",
        f"avg={avg_latency:.2f}ms, p99={p99:.2f}ms"
        if avg_latency < 500
        else f"avg={avg_latency:.2f}ms 超标!",
    )
    ...
    r.ok(
        "订阅者通知延迟",
        f"avg={avg_notify:.2f}ms" if avg_notify < 100 else f"avg={avg_notify:.2f}ms 偏高",
    )
    ...
    r.ok(
        "因果链查询延迟",
        f"avg={avg_chain:.2f}ms" if avg_chain < 100 else f"avg={avg_chain:.2f}ms 偏高",
    )
...
def test_classic_acceptance(r: TestResult):
    ...
    r.ok(
        f"C-Cost: 开销{overhead:.0f}%",
        f"baseline={baseline_time:.1f}ms, stream={stream_time:.1f}ms"
        if cost_pass
        else f"超标: {overhead:.0f}%",
    )
    ...
    r.ok(
        f"A-Accuracy: {accuracy:.0f}%",
        f"{tested_features}/{total_features}功能已验证" if acc_pass else "覆盖率不足",
    )
    ...
    stab_pass = cv < 15
    r.ok(
        f"S-Stability: CV={cv:.1f}%",
        f"rounds={[f'{x:.1f}' for x in rounds]}" if stab_pass else f"方差偏大: CV={cv:.1f}%",
    )
    ...
    r.ok("S-Security: 事件模型", f"frozen={'frozen' in str(event.model_config)}" if True else "")
    ...
    r.ok(
        "S-Security: Token映射",
        f"{len(AGENT_TOKENS)}个Agent" if len(AGENT_TOKENS) >= 5 else "Token不足",
    )
...
def test_edge_cases(r: TestResult):
    ...
    r.ok(
        "空EventStream",
        f"events={empty_stream.event_count}" if empty_stream.event_count == 0 else "非空",
    )
    ...
    for text, expected_type in special_texts:
        ...
        if parsed["event_type"] == expected_type:
            ...
            r.fail(
                f"特殊ICP: {text[:20]}",
                f"期望{expected_type.value}，得到{parsed['event_type'].value}",
            )

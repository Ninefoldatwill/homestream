...
>
> 开源版 V5.2.5 · 自进化AI生态操作系统（支持全模态实时语音交互）
>
> 融众之优，铸己之新。道法自然，由内而外。
...
<p align="center">
  ...
  <a href="#可观测性">可观测性</a> ·
  <a href="#路线图">路线图</a> ·
  <a href="docs/MEMORY_PHILOSOPHY.md">记忆哲学</a> ·
...
>
> 🖥️ **硬件配置**：请参阅 [硬件锚点指南](HARDWARE_GUIDE.md) 了解六大硬件档位（8GB→256GB+），找到最适合你的 AI 生态园配置。
>
...
│   ├── causal_memory.py       # 因果记忆桥接 (EventStream因果链↔记忆召回)
...
三引擎联动：**遗忘**（认知衰退）→ **合并**（聚类压缩）→ **重构**（反思提炼），让 Agent 越用越聪明。

| 记忆类型 | 认知科学对应 | 衰减半衰期 | 典型内容 |
|:---------|:-------------|:-----------|:---------|
| Reflective | 元认知 | 693天 | 核心洞察、哲学感悟 |
| Semantic | 陈述性记忆 | 138天 | 技术知识、事实 |
| Procedural | 程序性记忆 | 86天 | 操作流程、工作流 |
| Episodic | 情景记忆 | 46天 | 对话记录、事件 |
| Emotional | 情感记忆 | 34天 | 用户偏好、情绪标记 |

> **冻结主模型——零灾难性遗忘风险**：记忆演化完全不触碰模型参数，不需要 GPU/LoRA/训练数据。每条记忆是明文，可溯源、可审计、可修改。这是"普大众化"的底气——零成本、零风险、完全本地。
>
> **因果记忆闭环**：V5.0 新增 `CausalMemoryBridge`，将 EventStream 因果链与记忆演化系统桥接。事件发布时自动创建因果记忆（念起），检索时沿因果链追溯并加成（涌现），从记忆回溯到根事件（溯源）——形成"起心动念→因果循环→自然涌现"的完整闭环。
>
> 详细设计哲学见 [docs/MEMORY_PHILOSOPHY.md](docs/MEMORY_PHILOSOPHY.md)
...
#### V5.0.0 ✅ 已发布 (2026-06-29)

- 三层模型路由（L1本地 / L2云端 / L3备份，**永远免费托底**）
- EventStream 因果链 + 850+ 测试全通过
...
- 因果记忆闭环（EventStream因果链 ↔ 记忆召回）

#### V5.1.0 ✅ 已发布

- HS-UI 协议（Agent 驱动的声明式 UI 描述）
- 自适应接口模式（设备/角色/场景检测）
- 主题无障碍审计器（WCAG 2.1 AA 级配色审计）
- A2A 协作协议规范（基于 ICP v1.1）
- 可观测性前端（10 面板仪表盘）

#### V5.2.0 ✅ 已发布

- VoiceBridge 语音模块（FunASR 2-pass STT + 自托管 LiveKit）
- 浏览器端语音交互 demo
- 47+ 语音相关单元测试
...
- VoiceBridge 端到端联调完成：FunASR STT + Ollama qwen2.5:3b + EdgeTTS，自托管 LiveKit
- CosyVoice2 增强（独立 TTS 微服务，本地 GPU 音色）
- 多轮对话断流修复
- 语音链路可观测性
- **待完成**：双源提交 + 补 v5.2.5 Release Notes

### 未来计划

#### V5.3.0 计划中

- Tauri 2.0 桌面/移动 App 封装（原 V5.1.0 计划迁移至此版本）
- 跨平台本地运行壳

#### V5.3.5 计划中

- 可视化工作流编排器（拖拽式 Agent 工作流，原 V5.2.0 计划迁移至此版本）
- 可观测性主题面板

#### V5.4.0 计划中

- 浏览器自动化插件市场（原 V5.2.0 计划迁移至此版本）
- MCP + A2A 双协议生态互联

#### V5.4.5 计划中

- 一图一世界（Photo to Theme）— 拍一张照片，生成独一无二的 UI 主题
- 好玩项目（书阁 / 创意场景）
- 社区贡献体系与主题市场

### 未来愿景

- 🧠 **因果记忆引擎** — 从"检索驱动"到"因果驱动"：记忆沿因果链自然涌现，无需显式检索
- 🌐 多语言生态（i18n）
- 📡 MCP + A2A 双协议生态互联
...
**免责声明** — 见 [DISCLAIMER.md](DISCLAIMER.md)（AI输出准确性、第三方服务、不构成专业建议等）
